...
**首次运行**用 Tab 顶部 Stage Card 的「运行」按钮（批量生成所有内容）。

**重跑时必须走最小原子粒度**，禁止默认触发整个 stage：

| Tab | 原子单元 | 重跑入口 |
|---|---|---|
| 人物 | 单个角色（front + side + back） | 角色卡片上的「↺ 重跑全部角度」按钮 |
| 帧 | 单个 shot 的单个 frame_type | Shot 卡片内的首帧/末帧「↺ 重新生成」按钮 |
| 视频 | 单个 shot | Shot 卡片内的「↺ 重新生成视频」按钮 |
| 描述 | 单个 shot | Shot 卡片内的单独重跑入口（需实现时参照此原则） |

Stage Card 顶部的「强制重跑」仅用于需要**批量重置整个阶段**的场景，不是日常重跑的入口。新增 Tab 中的重跑功能时，**必须先在 item 级别提供入口，Stage 级别按钮为可选补充**。
